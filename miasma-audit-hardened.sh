...
progress() {
  if [ -t 2 ]; then
    printf '\r  [%s %d/%d] %s\033[K' "$3" "$1" "$2" "$4" >&2
  else printf '  [%s %d/%d] %s\n' "$3" "$1" "$2" "$4" >&2; fi
}
...
classify_binding_gyp() {
  ...
  for pkg in "${AFFECTED_PKGS[@]}"; do
    case "$f" in */node_modules/"$pkg"/*)
      reason="inside affected package $pkg"
      sev="!!"
      ;;
    esac
  done
  if [ "$sev" = "i" ] && grep -RIEqs "$MARKERS" "$pkgdir" 2>/dev/null; then
    reason="worm marker in same package dir"
    sev="!!"
  fi
  if [ "$sev" = "i" ] && [ "$sz" -gt 0 ] 2>/dev/null && [ "$sz" -le 200 ] 2>/dev/null; then
    reason="suspiciously tiny (~157B dropper tell)"
    sev="!"
  fi
    ...
    for pkg in "${AFFECTED_PKGS[@]}"; do
      ...
      if grep -RqsF -- "$pkg" "${manifest_files[@]}" 2>/dev/null; then
        ...
        if [[ -n "${BAD_VERSIONS[$pkg]:-}" ]]; then
          rvers=()
          while IFS= read -r rv; do [ -n "$rv" ] && rvers+=("$rv"); done \
...
implant_scan() {
  ...
  while IFS= read -r f; do
    ...
    case "$sev" in
    "!!")
      report "[!!] binding.gyp ($reason, ${sz}B): $f"
      copy_evidence "$f"
      ;;
    "!")
      report "[!] binding.gyp ($reason, ${sz}B): $f"
